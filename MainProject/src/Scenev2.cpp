...
bool Scene::LoadSkybox(const std::vector<std::string> &faces){
    stbi_set_flip_vertically_on_load(false);
    glGenTextures(1, &skybox);
    glBindTexture(GL_TEXTURE_CUBE_MAP, skybox);

    bool ok = true;
    ok = ok && load_cube_map_side(skybox, GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, faces[0].c_str());
    ok = ok && load_cube_map_side(skybox, GL_TEXTURE_CUBE_MAP_POSITIVE_Z, faces[2].c_str());
    ok = ok && load_cube_map_side(skybox, GL_TEXTURE_CUBE_MAP_POSITIVE_Y, faces[5].c_str());
    ok = ok && load_cube_map_side(skybox, GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, faces[1].c_str());
    ok = ok && load_cube_map_side(skybox, GL_TEXTURE_CUBE_MAP_NEGATIVE_X, faces[4].c_str());
    ok = ok && load_cube_map_side(skybox, GL_TEXTURE_CUBE_MAP_POSITIVE_X, faces[3].c_str());
    if(!ok) return false;
    ...
    for(const auto& mesh : model.meshes){
        ...
        for(const auto &prim : mesh.primitives){
            ...
            for(const auto &pair : prim.attributes){
                ...
                //if(bs != 0) std::cout << "bs = " << bs << "\n";
                //if(accessor.byteOffset) std::cout << "boffs " << accessor.byteOffset << "\n";
...
bool Scene::LoadEnvMap(const char* path){
