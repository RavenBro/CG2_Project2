...
Mesh *calculateAdjacencyData(const char *path)
{
...
void SVShadowPass::Init2(int w,int h)
{
...
void SVShadowPass::SetShaders(ShaderProgram *first, ShaderProgram *second, ShaderProgram *third)
{
    First = first;
    Second = second;
    Third = third;
}
void SVShadowPass::Render(float4x4 view, float4x4 projection, float3 campos)
{
...
void SVShadowPass::drawNode(int nodeId, const float4x4& model){
...
void SVShadowPass::draw(const Mesh::Primitive& prim, const float4x4 &model){
...
void SVShadowPass::OnInit()
{
  ...
  for (int i=0;i<9;i++)
  {
      //if (i==0) continue;
      //if (i%2) continue;
      if (i==5) a = translate4x4(float3(-0.001,-0.001,-0.002));
    else a = translate4x4(float3(0.001,-0.002,0.001));
    auto str = "s"+std::to_string(i)+".txt";
    m =(PrimitiveMesh *)calculateAdjacencyData(str.c_str());
  ...
  for (int i=0;i<ls.GetCount();i++)
  {
      ...
      lights.push_back(l.position);
      max_shadow_len.push_back(l.shadow_len);
  }
}
void SVShadowPass::Process()
{
    //lights[0] = GetScene()->sceneCamera.pos+float3(0,0.3,1.001);
