...
void wedge_proj(vec3 u1,vec3 u2,vec3 light_pos,float radius,float scale)
{
  ...
  //if (dec>1) vv1/=dec;
  //else vv2*=dec;
  vec3 vvm = 0.5*vv1+0.5*vv2;
  float l = scale/length(vvm);
  vec3 edge_v = u2 - u1;
  vec3 rv1 = normalize(cross(edge_v,vv2))*radius;
  rv1 =rv1 * l;

  vec3 rv2 = -normalize(vvm)*radius;
  rv2 = rv2 * l;
...
void wedge_proj_old(vec3 u1,vec3 u2,vec3 light_pos,float radius,float scale)
{
  vec3 vv1 = u1 - light_pos;
  vec3 vv2 = u2 - light_pos;
  float dec = (length(vv1)/length(vv2));
...
void calc_wps(vec3 u0,vec3 u1, vec3 u2, vec3 u3,vec3 light_pos,float radius,float scale)
{
  ...
  if (length(cr)<EPS)//точки u0 u1 u2 лежат на одной прямой
  {
...
void wedge_create(vec3 u1, vec3 u2,vec3 light_pos,float radius,float scale)
{
  ...
  //wedge_proj_old(u1,u2,light_pos,radius,scale);
  wedge_proj(u1,u2,light_pos,radius,scale);
...
void main()
{
    ...
    if (c>EPS)
    {
      ...
      vec3 v40sh = normalize(v4.xyz - v0.xyz)/5000.0 + v0.xyz;
      vec3 v04sh = normalize(v0.xyz - v4.xyz)/5000.0 + v4.xyz;

      vec3 v20sh = normalize(v2.xyz - v0.xyz)/5000.0 + v0.xyz;
      vec3 v02sh = normalize(v0.xyz - v2.xyz)/5000.0 + v2.xyz;

      vec3 v42sh = normalize(v4.xyz - v2.xyz)/5000.0 + v2.xyz;
      vec3 v24sh = normalize(v2.xyz - v4.xyz)/5000.0 + v4.xyz;
      if (s1&&(side==1))
      {
        cl = vec4(1.0,0.0,0.0,1.0);
      ...
      {
        cl= vec4(0.0,0.0,1.0,1.0);
        wedge_create(v40sh,v04sh,light,light_radius,SC);
      }
    }
